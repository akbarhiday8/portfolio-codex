...
export default function ProjectEdit() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const [projectRes, toolsRes, categoriesRes] = await Promise.all([
        ...
        const projectData = projectRes?.data?.data ?? projectRes?.data ?? null;
        setProject(projectData);
        setTools(toolsRes?.data?.data ?? toolsRes?.data ?? []);
        setCategories(categoriesRes?.data ?? []);
        setSelectedTools(projectData?.tools?.map((item) => item.id) ?? []);
  ...
  const submit = handleSubmit(async (values) => {
    try {
      ...
      if (newImages.length) {
        ...
        newImages.slice(0, 7).forEach((file) => formData.append('images[]', file));
      ...
      const message = error.response?.data?.message ?? 'Gagal memperbarui proyek';
      toast.error(message);
    }
  });
  ...
  if (!project) {
    return <div className="rounded-lg bg-white p-6 shadow">Proyek tidak ditemukan.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Edit Proyek</h2>
          <p className="text-sm text-gray-500">Perbarui informasi proyek dan kelola gambar.</p>
        </div>
      </div>
            ...
            {project.images?.map((image) => (
              <div key={image.id} className="relative overflow-hidden rounded-lg border">
                ...
                <button
                  ...
                  className="absolute right-2 top-2 rounded bg-red-500 px-2 py-1 text-xs font-semibold text-white hover:bg-red-600"
                >
                  Hapus
                </button>
              </div>
            ))}
            {!project.images?.length && <p className="text-sm text-gray-500">Belum ada gambar.</p>}
          </div>
        </div>

        <MultipleUpload files={newImages} setFiles={setNewImages} limit={7} />
